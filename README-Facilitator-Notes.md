...
- The training certificate defaults to 12 months (`-CertificateValidityMonths` accepts 1–1000), is copied to Current User > Personal with .NET `X509Store`, and is imported without the `Exportable` flag. Certificate/key file extensions are Git-ignored; setup exports no certificate files.
